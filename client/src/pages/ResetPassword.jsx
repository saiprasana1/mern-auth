...
const ResetPassword = () => {
  ...
  const inputRafs = React.useRef([])
  
    const handleInput = (e, index) =>{
       if(e.target.value.length > 0 && index < inputRafs.current.length - 1 ){
        inputRafs.current[index + 1 ].focus();
       }
    }
    ...
    const handlePaste = (e) =>{
      ...
      pasteArray.forEach((char,index)=>{
        inputRafs.current[index].value = char;
      })
    ...
    const onSubmitEmail = async (e)=>{
       e.preventDefault();
      try {
      const {data} =  await axios.post(backendUrl + '/api/auth/send-reset-otp', { email: email.toLowerCase() })
        console.log("🟢 Response from /send-reset-otp:", data);
        data.success ? toast.success(data.message) : toast.error(data.message)
        data.success && setIsEmailSent(true)
      } catch (error) {
        toast.error("something went wrong")
          console.error("🔴 Error occurred in send-reset-otp:", error);
      }
    }

    const onSubmitOtp = async (e)=>{
      e.preventDefault();
      const otpArray =inputRafs.current.map(e => e.value)
...
{!isEmailSent && 
       <form  onSubmit={onSubmitEmail} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        ...
        <div className='mb-4 flex px-5 py-2.5 items-center gap-3 w-full rounded-full bg-[#333A5C] '>
       ...
       <form onSubmit={onSubmitOtp} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
          ...
          <div className='flex justify-between mb-8' onPaste={handlePaste}>
            {Array(6).fill(0).map((_,index)=>(
              ...
              ref={e => inputRafs.current[index] = e }
